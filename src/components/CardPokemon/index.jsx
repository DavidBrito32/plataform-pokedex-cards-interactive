...
const CardPokemon = ({pokemon, btn}) => {
  ...
  const { addPokedex, delPokemon, setPokeDetails, alteraPage, pokeDetails } = useContext(HooksContext);
  ...
  useEffect(() => {
    if (pokeInfo && pokeInfo.types && pokeInfo.types.length > 0 && pokeInfo.types[0].type.name) {
    ...
    if (pokeInfo && pokeInfo.types && pokeInfo.types.length > 1 && pokeInfo.types[1] && pokeInfo.types[1].type.name) {
        ...
        <>
            ...
            >
                <div className="pokeInfos">
                    ...
                    <div className="types">
                        <span className='type1'><img src={imageType1} alt="TIPO 1" /> {type1} </span>
                       {type2 &&  <span className='type2'><img src={imageType2} alt="TIPO 2" /> {type2} </span>}
                    </div>

                    <div className="action-Button">
                        <Link onClick={openDetails} to={`/pokedex/details/${convertNumber(pokeInfo && pokeInfo.id)}`}>Detalhes</Link>
                        {
                          btn ? 
                        (<span className='excluir' onClick={() => delPokemon(pokemon)}>Excluir</span>) :
                    ...
                    <div className="images">
                        ...
                        {pokeInfo && <img src={`https://www.serebii.net/swordshield/pokemon/${convertNumber(pokeInfo.id)}.png`} className='pokeImage' alt="Pokemon" />}
                    </div>
                </div>
            </CardContainer>
        </>
